...
class UltrafastLaneDetector():
	...
	def initialize_model(model_path, cfg, use_gpu):
		...
		net = parsingNet(pretrained = True, backbone='18', cls_dim = (cfg.griding_num+1,cfg.cls_num_per_lane,4),
	...
	def process_output(output, cfg):		
		...
		max_len = max(len(lane) for lane in lanes_points)
		for lane in lanes_points:
			while len(lane) < max_len:
				lane.append([])
	...
	def draw_lanes(input_img, lanes_points, lanes_detected, cfg, draw_points=True):
		...
		if(lanes_detected[1] and lanes_detected[2]):
			...
			# Filter out empty lists from lanes_points
			valid_lane_1_points = [point for point in lanes_points[1] if point]
			valid_lane_2_points = [point for point in lanes_points[2] if point]

			if valid_lane_1_points and valid_lane_2_points:
				pts = np.vstack((valid_lane_1_points, np.flipud(valid_lane_2_points)))
				cv2.fillPoly(lane_segment_img, [pts], color=(0, 191, 0))
		...
		# # Add text to indicate if lanes are detected
		# lane_status_position = 50
		# for lane_num, detected in enumerate(lanes_detected):
		# 	if lane_num < len(lanes_points) and np.any(lanes_points[lane_num]):  # Check if there are points for this lane
		# 		text = "Lane detected" if detected else "Lane not detected"
		# 		color = (0, 255, 0) if detected else (0, 0, 255)
		# 		position = (50, lane_status_position)  # Adjust position as needed
		# 		lane_status_position += 30  # Move down for next lane status

		# 		cv2.putText(visualization_img, text, position, cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2, cv2.LINE_AA)
  
		# Check if any lane is detected
		any_lane_detected = np.any(lanes_detected)
		text = "Lanes detected" if any_lane_detected else "Lanes not detected"
		color = (0, 255, 0) if any_lane_detected else (0, 0, 255)
		position = (50, 50)
